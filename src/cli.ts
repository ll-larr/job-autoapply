 ...
 *
 * Изначально здесь стояла установка `process.env.NODE_USE_ENV_PROXY ??= '1'`
 * в самом начале файла — так, как это сформулировано в задании. Проверка
 * (не поверить на слово, а измерить) показала, что на этой сборке Node
 * (v24.14.0) переменной окружения с таким именем не существует вовсе:
 * `process.env.NODE_USE_ENV_PROXY = '1'`, выставленная из уже запущенного
 * процесса — хоть первой строкой файла, хоть раньше — не включает ничего,
 * ни при каком порядке импортов. Живой флаг называется `--use-env-proxy`,
 * это флаг командной строки Node, а не переменная окружения, и он
 * потребляется нативным бутстрапом процесса до того, как выполнится хоть
 * одна строка пользовательского JS — то есть в принципе не может быть
 * включён кодом изнутри уже стартовавшего процесса.
 *
 * Проверено эмпирически (два закольцованных loopback-сервера вместо цели и
 * прокси, `NODE_DEBUG=undici` в трассе): без флага undici идёт к цели
 * напрямую; с флагом — сперва CONNECT на прокси, потом туннель до цели.
 * `tsx` пробрасывает незнакомые ему флаги в `execArgv` дочернего процесса
 * node, так что `tsx --use-env-proxy src/cli.ts <команда>` включает его по
 * факту — это подтверждено тем же `NODE_DEBUG=undici`. Полная трасса — в
 * .superpowers/sdd/task-13-report.md.
 *
 * Поэтому флаг стоит в npm-скриптах (package.json: `tsx --use-env-proxy
 * src/cli.ts ...`), а не в теле этого файла — телом файла его включить
 * нельзя. Ниже остаётся только диагностика на случай прямого запуска в
 * обход npm-скриптов (см. warnIfProxyFlagMissing).
 */
...
function warnIfProxyFlagMissing(): void {
  if (!process.execArgv.includes('--use-env-proxy')) {
    console.error(
      'ВНИМАНИЕ: процесс запущен без --use-env-proxy. Если провайдер блокирует ' +
